...
export default function CustomerChart() {
  ...
  const totalCustomers = data.reduce(
    (sum, item) => sum + item.totalCustomer,
    0,
  );
  const avgCustomers =
    data.length > 0 ? (totalCustomers / data.length).toFixed(1) : 0;
                ...
                >
                  ...
                  >
                    {data.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={getBarColor(entry.totalCustomer)}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-between text-sm">
            <p className="text-gray-600">
              Showing data for{" "}
              <span className="font-bold text-[#8B2E1F]">{selectedYear}</span>
            </p>
