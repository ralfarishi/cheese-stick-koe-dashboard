...
export default function AddIngredientModal({
	...
	const {
		...
		watch,
		formState: { errors },
	} = useForm<AddIngredientFormValues>({
		defaultValues: {
			name: "",
			unit: "gr",
			costPerUnit: "",
	...
	const handleCalcChange = (field: keyof CalcData, value: string): void => {
		...
		if (!isNaN(p) && !isNaN(q) && q > 0) {
			const unitCost = (p / q).toFixed(2);
			setValue("costPerUnit", unitCost, { shouldValidate: true });
		}
	};

	const onSubmit = async (data: AddIngredientFormValues): Promise<void> => {
		setIsLoading(true);
		try {
