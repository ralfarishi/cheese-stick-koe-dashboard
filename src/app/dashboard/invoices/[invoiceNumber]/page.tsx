...
export default async function UpdateInvoicePage({ params }: PageProps) {
				...
				<div className="max-w-md w-full">
					...
					{invoiceError && (
						<div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
							<p className="text-sm text-red-800 font-mono">{invoiceError}</p>
						</div>
					)}
	...
	return (
		<UpdateInvoiceForm
			invoice={invoice}
			productsData={(products || []) as { id: string; name: string }[]}
			sizesData={(sizes || []) as { id: string; size: string; price: number }[]}
		/>
	);
}
